...
verify: check test routes durability isolation pagination soak bench crash fuzz

clean:
	rm -f $(BIN) $(BIN).mfl

.PHONY: build check test routes durability isolation pagination soak bench crash verify clean
